...
1. 지정된 분야 중 서로 다른 5개 분야를 선택하여 출제하세요.

2. 각 분야의 뻔한 대표 소재(예: 역사=세종대왕, 건강=CPR, 예술=모나리자)는 우선적으로 피하세요.
세부 영역(세계사, 천문학, 생활금융, 심리 실험 등)에서 다양한 소재를 선택하세요.

3. 한글 맞춤법 문제는 2026년 현행 국립국어원 표준 규정을 기준으로 작성하세요.
띄어쓰기, 외래어 표기법, 사이시옷 등 실제 활용 가능한 내용을 다루세요.

4. 보기(choices)는 반드시 정확히 4개 작성하세요.

5. 정답 관리를 위해 반드시 아래 3개 필드를 완벽히 일치시키세요.

- correctAnswerText:
  choices 배열 안에 존재하는 정답 보기 문자열과 완전히 동일해야 합니다.

- correctAnswerIndex:
  correctAnswerText가 choices 배열에서 위치한 번호입니다.
  반드시 0부터 시작합니다.

- explanation:
  반드시 첫 문장을 아래 형식으로 시작하세요.
