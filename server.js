...
조금이라도 오류 있을 시 REJECT{valid: false}하시오.
...
2. 개념 혼동 및 시간·장소의 논리 확정성 검증:
   - 개념 및 층위 혼동 검증:
     * 추상적 표준/명세(Specification)의 개념과 특정 OS/컴파일러/실제 구현체(Implementation)의 개념이 섞여 있는지 확인합니다.
     * 상관관계와 인과관계를 혼동했거나, 단순 선후 관계를 직접적 인과관계로 비약했는지 확인합니다.
   - 시간적 시차(Lag) 및 순서의 논리성 검증:
     * 원인 발생 후 결과가 나타나기까지의 시차(Lag)를 무시하고 '즉시', '가장 먼저' 일어나는 현상으로 오기했는지 검증합니다.
     * 특정 시점(연도, 시대, 법 개정 전후)에 따라 사실관계가 달라지는 항목인지 확인합니다.
   - 장소·환경·시점의 범위 확정성 검증:
     * 국가별/기관별(예: 한국은행 vs 미국 연준, 국내법 vs 해외법)로 다른 기준을 보편적 사실처럼 서술했는지 검증합니다.
     * 특정 플랫폼/환경(예: Linux vs Windows, 32bit vs 64bit)에서만 성립하는 특성을 일반화했는지 확인합니다.
   - 절대적 수식어 역검증:
     * '가장 먼저', '반드시', '모두', '직접적으로' 등의 절대적 표현이 쓰인 경우, 단 하나의 반례나 예외 조건이라도 존재하면 즉시 실패(valid: false) 처리합니다.
     
3. 오답지(Distractor) 및 정답 역검증:
   - 정답의 타당성뿐만 아니라, 나머지 오답지들이 어떤 해석이나 조건에서도 정답이 될 수 없음을 개별 증명합니다.
   - 문제 전제조건이 부실하여 오답지가 정답으로 인정될 여지가 0.1%라도 있는지 확인해야 하며, 그러한 여지가 존재하면 즉시 실패 처리합니다.
...
- 검증 결과, 문제·보기·해설·출처 중 단 하나라도 이상/오류/모호함이 발견되면:
   { "valid": false, "reason": "발견된 구체적 오류 및 이유 상세 작성"}
- 오류가 0%라고 완전하게 확신할 때만:
   { "valid": true, "reason": "오류가 없고 정확한 이유 상세 작성"}
