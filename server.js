...
- [절대 금지] 한자(漢字) 및 중국어 표현(예: 最重要的是 등)을 절대로 포함하지 말고, 순수 한국어(ko-KR)로만 작성하시오. 영어는 보충설명용으로 괄호 안에만.
...
2. 한글 맞춤법 문제:
- 현행 표준 규정 기준으로 다양한 세부 카테고리에서 낼 것.
- 괄호 설명, 추가 부연 설명 금지.
- 띄어쓰기 문제는 각 선택지가 실제 사용 가능한 표현이어야 하며, 마침표 앞 공백은 절대 금지.
...
4. choices:
- 반드시 4개이며 모든 선택지는 서로 달라야 한다.
- 정답 외 3개 오답은 질문의 전제나 범위에 '절대 부합하지 않는 확실한 오답'이어야 한다.
- 정답 처리가 가능해질 위험이 있는 유사 개념을 오답으로 선택하지 마시오.
...
6. explanation:
- [문법 자동 수정 엄격 금지]
  correctAnswerText 위치에는 choices에 있는 문자열을 토시 하나 안 바꾸고 단순 복사(Copy & Paste)하여 삽입하시오.
  한국어 문법상 아무리 어색하고 틀린 문장이 되더라도 종결어미, 조사, 단어를 절대로 교정/수정/윤문하지 마시오. (문법이 어색한 것이 정상입니다.)

- 첫 문장 포맷: 정답은 {correctAnswerText}입니다.
- 이후 오답 선택지가 틀린 이유를 choices 배열 순대로 설명한다. '~은 틀리다','~은 아니다' 같은 단순 부정문 할루시네이션 금지. (전체 4문장 이상)
...
- 4개 보기 중 정답으로 해석될 여지가 있는 보기가 2개 이상 존재하지 않는가?
- explanation에서 오답이 정답 조건에 맞다고 설명하고 있지 않은가?
