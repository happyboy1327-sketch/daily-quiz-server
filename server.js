...
2. 개념·시간·장소·조건 검증(문제·정답·해설 모두 해당함)
   ...
   - '가장 먼저', '반드시', '모두', '직접적으로', '...기준으로 가장 긴/ 가장 넓은' 등 절대적 표현은 단 하나의 반례나 예외가 있어도 valid=false 처리합니다.
   - ★ [한글 맞춤법 해설에서 필수] 단어의 표기 원리나 이유를 설명할 때, "실제 음운 현상(발음)"과 "해당 조항의 조건(원인)"이 언어학적으로 100% 일치하는지 검증하십시오.
     - 예: 발음은 'ㄴ' 소리가 덧나는데(예: [나문닙]), 해설에서 '된소리(경음)가 덧나기 때문'이라고 원인을 잘못 매칭한 경우, 출처 URL이 아무리 정확해도 인과관계 오류이므로 즉시 REJECT(valid: false) 처리합니다. 이 경우, errorType은 GRAMMAR_LOGIC_ERROR, PREMISE_MISMATCH 입니다.
...
3. 스포일러 및 실수 방지를 위한 문제 검증
   - 문제의 질문에 정답 그 자체 또는 정답을 직접 특정할 수 있는 용어, 핵심어, 제목, 고유명사, 힌트 등이 포함되어 정답을 바로 노출하는 경우 false 처리하시오.
   - 질문이 정의문에 맞는 특정 용어, 명칭, 이름, 개념, 인물, 장소, 작품명 등을 묻는 경우, correctAnswerText에는 반드시 질문에서 요구하는 바만 들어가야 합니다. 그 정의나 설명문을 정답으로 제시하면 false 처리하시오.
   → 질문이 "무엇이라고 합니까?", "무엇입니까?", "어떤 용어입니까?" 등 특정 용어 또는 명칭을 요구하는 형태라면, 정답은 반드시 해당 용어 또는 명칭 자체여야 하며 정의문이나 설명문이어서는 안 됩니다.
   - 질문이 정의 또는 현상을 설명하고 그에 해당하는 용어를 묻는 형태라면, correctAnswerText는 반드시 그 정의에 해당하는 정확한 용어여야 합니다.
   - 반대로 질문 자체가 정의, 설명, 이유, 원인 등을 묻는 경우에는 완전한 설명문을 정답으로 사용할 수 있습니다.
   - 질문의 요구 유형과 correctAnswerText의 답변 유형이 서로 맞지 않으면 false 처리하시오.
...
5. 정답 및 오답지 역검증 II (rigorous)
   ...
   - AI가 만든 정답만 질문의 전제 및 조건에 부합하는지 검사할 뿐만 아니라, 다른 선택지는 질문의 전제 및 조건에 부합하지 않는지도 검사해야 합니다.

6. 비판적 심문(Red Teaming)
 ...
 */
    ...
    const safeContextPatterns = [
        /오류가\s*없고\s*복수\s*정답\s*가능성(?:도|이|은)\s*없(?:으며|다|습니다)/,
        /선택지\s*간[\s\S]*?중복이나 복수\s*정답\s*가능성(?:도|이|은)\s*없(?:으며|다|습니다)/,
        /오류가\s*없고\s*중복이나\s*복수\s*정답\s*가능성(?:도|이|은)\s*없(?:으며|다|습니다)/,
        /검토했(?:으나|지만)/,
        /정답이\s*아닙니다/,
        /일치하고 복수\s*정답 가능성(?:이|은|도) 없(?:으며|다|습니다)/,
        /일치하고 복수\s*정답 문제(?:가|는|도) 없(?:으며|다|습니다)/,
        /(?:유일하게|단\s*하나|1개뿐)/,
        /정답은\s*하나/,
        /정답은\s*한 개/,
        /모두[\s\S]*?(?:정확|유일)합니다/,
        /가능성(?:이|도|은) 없(?:습니다|다|으며|고)/,
        /(?:출처)\s*오류가 없(?:습니다|다|으며|고)/,
        /이상(?:은|도|이) 없(?:으며|고|다|습니다)/, 
        /문제(?:도|는|가) 없(?:다|으며|습니다)/
    ];
